...
- スクリーン -&gt; 制御コード
  - 「エスケープシーケンス」グループボックスにある「No. ?67 BSキーでBSを送信 BSキーでDELを送信」のチェックを外す。
     > チェックが入っていると、BSキーが ^H となり、C-h キーと同じ挙動となる。
     > チェックを外すと、BSキーが ^? となり、C-h キーと異なるマッピングが可能になる。vimの設定で重要。
  - 「エスケープシーケンス」グループボックスにある「No. ?8428 Aタイプを半角で表示 Aタイプを全角で表示」のチェックを外す。
     > チェックが入っていると、UnicodeのAタイプ(Ambiguous)の文字幅を半角で表示する。
     > チェックを外すとUnicodeのAタイプ(Ambiguous)の文字幅を全角で表示する。
- スクリーン -&gt; ヒストリー
...
- キーボード -&gt; キーコード
  > ドロップダウンを「0」にするか「2」にするか検討中。「2」にするだけで、microでShiftキー＋カーソルキーで範囲選択が有効になる。  
  > ただし、vim側におけるキーマッピングの動作確認が必要となる。
  
  - 「キーコードの変更」グループボックスで、「1. modifyCursorKeys」でINSERTを削除し、ドロップダウンを「0」にする。
    > デフォルトは、```UP,DOWN,RIGHT,LEFT,PRIOR,NEXT,HOME,END,INSERT,DELETE``` について 「-1」  
    > これを、　　　```UP,DOWN,RIGHT,LEFT,PRIOR,NEXT,HOME,END,DELETE``` について 「0」にする。  
    > INSERTを除去しない状態で、「modifyKeysがメニューのショートカットキー設定より優先する」にチェックを入れると、Shift-INSERTでのペーストができなくなってしまう。
  - 「キーコードの変更」グループボックスで、「2. modifyFunctionKeys」で、ドロップダウンを「0」にする。
    > デフォルトでは、```F1-F12``` について 「-1」。キーの変更は必要ない。
  - 「キーコードの変更」グループボックスで、「3. modifyKeypadKeys」で、ドロップダウンを「0」にする。
